...
import { useSignInMutation } from "@/controller/api";
import Link from "next/link";
import { useDispatch } from "react-redux";
import { setCredentials } from "@/controller/authSlice";
import { ISignin } from "@/controller/types";
...
export default function SignInPage() {
  ...
  const handlesignInFormSubmit = async (
    ...
    e.preventDefault();
    try {
      const user = await signIn(signInForm).unwrap();
      ...
      </div>
      {
        ...
        tab === "email" ? (
          ...
          >
            ...
            {/* Sign up button */}
            <button type="submit" className="btn-blue py-3 px-8 w-full">
              Sign In
            </button>
            {/* Login redirect */}
            <p className="text-center text-[#8e8f94] font-light pt-2">
              Don&apos;t have an account?{" "}
              <Link href={"/auth"} className="text-yara-blue font-medium">
                Sign Up
              </Link>
            </p>
